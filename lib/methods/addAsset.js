...
const handleDetails = (data, fail, storage, clientModules, hostData, dataCallback, errorCallback) => function (details) {
  if (typeof details.mode !== 'string') {
    ...
    if (clientModules.hasOwnProperty(mode)) { // Client Module was already retrieved
      return this.initAsset(initData, dataCallback, errorCallback);
    } else if (data.hasOwnProperty('clientModuleCodeBlob')) { // Use provided  clientModuleBlob
      return this.initAsset({...initData, clientModuleCodeBlob: data.clientModuleCodeBlob, check: data.check}, dataCallback, errorCallback);
      ...
      compareHashes(mode, hostData, initData, dataCallback, errorCallback, storage).bind(this),
      errorCallback
      );
    } else { // fetch clientModule blob from host
      return loadBlobFromRemote(mode, hostData, initData, dataCallback, errorCallback, null, null).bind(this)();
    }
  }
};
