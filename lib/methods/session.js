...
exports.session = (user_keys, fail) => function (data, dataCallback, errorCallback) {
  ...
  return this.logout({}, () => { // first logout clear current data
    ...
    if (typeof dataCallback === 'function') dataCallback(data.username); // TODO what to return if no username provided?
  });
};
...
function hexToKey (input) {
  ...
  let ordered = '';
  for (let i = 0; i < string.length; i++) ordered += string.charCodeAt(i);
  return ordered;
}
