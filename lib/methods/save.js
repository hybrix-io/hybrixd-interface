...
exports.save = (fail, powQueue, legacy) => function (data, dataCallback, errorCallback, progressCallback) {
  ...
  if (typeof data !== 'object') {
    ...
    const key = data.key;
    this.rout({host: data.host, query: '/e/storage/save/' + encodeURIComponent(key) + '/' + encodeURIComponent(stringifyAndEscapeQuotes(data.value)), channel: data.channel}, redirectCallback(key), errorCallback, progressCallback);
