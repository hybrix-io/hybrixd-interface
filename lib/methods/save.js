...
const handleWork = (key, data, powQueue) => function (result, dataCallback, errorCallback, progressCallback) {
  if (typeof result !== 'object' || result === 0) return errorCallback('Received illegal work result');
...
function saveRemote (storage, key, data, powQueue) {
  return [
    value => value.length < 200
      ? ({host: data.host, query: '/e/storage/save/' + key + '/' + value, channel: data.channel, meta: data.meta})
      : ({host: data.host, query: '/e/storage/save/' + key, data: value, channel: data.channel, meta: data.meta}),
    'rout',
...
/**
   ...
   * );
   */
exports.save = (storage, fail, powQueue, legacy, hasHost) => function (data, dataCallback, errorCallback, progressCallback) {
  if (typeof data !== 'object' || data === null) return fail('Expected an object.', errorCallback);
    ...
    if (data.legacy === true && legacy(data.key) === null) return fail('Legacy mode requires a session', errorCallback);
