...
/**
   ...
   * @param {Object} [data._options.failIfAllFail=true] - Whether fali stream if all steps fail
   * @param {Object} [data._options.failIfAnyFails=false] - Whether to fail entire stream if any step fail
...
exports.parallel = fail => function (data, dataCallback, errorCallback, progressCallback) {
  ...
  const errorSubCallback = i => error => {
    ...
    if (success || options.failIfAllFail === false) return dataCallback(resultData);
