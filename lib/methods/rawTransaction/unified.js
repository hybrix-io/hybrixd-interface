...
function validateConstructAndSignRawTransaction (asset, assets, data, dataCallback, errorCallback, progressCallback) {
  ...
  if ((targetArray.length === 2 && targetArray[0] === asset.symbol) || targetArray.length === 1) {
    ...
    data.target = decompressedAddress ? decompressedAddress : data.target;
  }
  ...
  console.log('sourceSubSymbols: '+JSON.stringify(sourceSubSymbols));
  ...
  console.log('targetSubSymbols: '+JSON.stringify(targetSubSymbols));
