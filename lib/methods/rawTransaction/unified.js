...
function validateConstructAndSignRawTransaction (asset, assets, data, dataCallback, errorCallback, progressCallback) {
  ...
  if (totalAvailableBalance.gte(data.amount)) {
    const followUp = createTransaction(assets, targetSubAddresses, availableBalances, data, dataCallback, errorCallback, progressCallback).bind(this);
    if (Object.keys(availableBalances).length === 1) {
      return followUp(availableBalances); // use availableBalances as stub for valuations. As this does not matter for singular assets.
      ...
      for (const subSymbol in availableBalances) {
        ...
        feeValuationSteps[subSymbol] = feeSymbol === valuationSymbol || (valuationSymbol === 'hy' && feeSymbol.endsWith('.hy'))
          ...
          : {
              data: {
                query: `/e/valuations/rate/${feeSymbol}/${valuationSymbol}/${fee}`,
                host: data.host,
                channel: data.channel
              },
              step: 'rout'
      ...
      return this.parallel(feeValuationSteps, followUp, error => {
