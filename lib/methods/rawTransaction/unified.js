...
function checkAvailableBalance (subSymbol, subResult, assets) {
  ...
  if (!regular.hasDifferentFeeSymbol(subAsset)) { // symbol === fee-symbol
    const availableBalance = new Decimal(subResult.balance).minus(subAsset.fee);
    if (availableBalance.isPositive()) return {balance: availableBalance.toString()};
    else {
  ...
  } else { // symbol !== fee-symbol
    const availableFeeBalance = new Decimal(subResult.feeBalance).minus(subAsset.fee);
    if (availableFeeBalance.isPositive()) {
      const availableBalance = new Decimal(subResult.balance).toString();
      return {balance: availableBalance}; // all balance is available
    } else {
...
const createTransaction = (assets, targetSubAddresses, availableBalances, data, dataCallback, errorCallback, progressCallback) => function (valuations) {
...
const checkAvailableBalances = (assets, targetSubAddresses, data) => function (subResults, dataCallback, errorCallback, progressCallback) {
  ...
  for (const subSymbol in subResults) {
  ...
  if (totalAvailableBalance.gte(data.amount)) {
    ...
    for (const subSymbol in availableBalances) {
      const subAsset = assets[subSymbol];
    ...
    this.parallel(feeValuationSteps, createTransaction(assets, targetSubAddresses, availableBalances, data, dataCallback, errorCallback, progressCallback).bind(this), errorCallback, progressCallback);
  } else {
...
function validateConstructAndSignRawTransaction (asset, assets, data, dataCallback, errorCallback, progressCallback) {
  ...
  const steps = {_options: {failIfAnyFails: true}};
  for (const subSymbol of targetSubSymbols) {
    const subAsset = assets[subSymbol];
