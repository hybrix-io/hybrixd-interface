  ...
  if (data.steps.length === 0) {
    ...
    if (typeof step === 'string') {
      if (this.hasOwnProperty(step)) {
        ...
        } else if (step === 'parallel') { // use previous data for 'parallel' method
          if (!data.hasOwnProperty('data')) data.data = {};
          if (!data.data.hasOwnProperty('_options')) data.data._options = {};
          if (!data.data._options.hasOwnProperty('data')) data.data._options.data = data.prevData;
        }
