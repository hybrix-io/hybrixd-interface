...
function addAssetAndUpdateBalanceSteps (data, symbol, assets) {
  return [
    {symbol}, 'addAsset',
    updateBalance(data, symbol, assets).bind(this), 'call'
  ];
}
...
function getUpdateBalanceSteps (data, symbol, assets) {
  if (assets.hasOwnProperty(symbol)) {
    ...
    if (isHyAsset(asset)) return updateHyBalancesSteps(data, symbol, asset);
...
function determineWhichAssetsToGet (assets, data) {
...
exports.refreshAsset = assets => function (data, dataCallback, errorCallback, progressCallback) {
  const assetList = determineWhichAssetsToGet(assets, data);

  const steps = {};
  for (let symbol in assetList) steps[symbol] = {data: getUpdateBalanceSteps.bind(this)(data, symbol, assets), step: 'sequential'};

  this.parallel(steps, handleUndefinedAndUnifiedBalances(assets, assetList, dataCallback, errorCallback, progressCallback).bind(this), errorCallback, progressCallback);
};
