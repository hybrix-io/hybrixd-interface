  ...
  */
    ...
    */
    ...
    const errorCallbackWithRetries = error => {
      ...
      try {
        ...
        if (parsedError.hasOwnProperty('error') && parsedError.hasOwnProperty('data') && !meta) return fail(parsedError.data, errorCallback);
        else return retryOrErrorCallback(parsedError);
      } catch (e) {
        return retryOrErrorCallback(error);
      }
    };
  ...
  this.yCall = (data, dataCallback, errorCallback) => {
    ...
    */
    ...
    return this.call({query, connector: data.connector, meta: true, retries: data.retries}, result => {
  ...
  this.zCall = (data, dataCallback, errorCallback) => {
    ...
    */
    ...
    return this.yCall({query: encodedQuery, channel: 'z', userKeys: data.userKeys, connector: data.connector, retries: data.retries}, compressedData => {
